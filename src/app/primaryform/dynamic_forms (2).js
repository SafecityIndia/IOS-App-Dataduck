...
function componentRadio(data, lastQuestion) {
	...
	data["options"].forEach(function (option) {
		...
		optionHtml += `
			...
			data["question"]["id"]
		}" class="custom-control-input dynamic-radio" data-val="${option.title}" value="${option.id}" ${
	...
	data["options"].forEach(function (option) {
		...
		elementHtml += `
			<div class="inputGroup custom-control shareincidentform">
			  <input type="checkbox" id="${option.id}" data-id="1"  name="option${data["question"]["id"]}" class="custom-control-input getAttr dynamic-checkbox" value="${option.id}" data-parentid="${option.parent_id}" data-ismain="${option.is_main}" data-val="${option.title}" data-showtextbox="${showTextbox}" ${isSelected?"checked":""}>
